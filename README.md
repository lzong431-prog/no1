...
<header class="top">
  <div class="bar">
    <div class="statebar">
      ...
      <div id="stateText">等待操作</div>
    </div>
    <div class="right">
      <button id="openDrawer" class="primary" style="padding:6px 10px">查看命中日志</button>
      <div>下次自动更新：<span id="countdown">--</span></div>
      <div>上次成功：<span id="lastOk">--</span></div>
      <div>错误计数：<span id="errCnt">0</span></div>
    </div>
  </div>
</header>
...
<div class="wrap">
  <h1>马上发财
    <span class="badge">稳住</span>
    <span class="badge">能赢</span>
  </h1>
  ...
  <div class="mainGrid">
      ...
      <section class="card">
        <div class="toolbar">
          ...
          <div>
            ...
            <select id="mode">
              ...
              <option value="proxy">通过代理</option>
              <option value="paste">手动粘贴 JSON</option>
            </select>
          </div>
          <div>
            <label>计划码数量 K</label>
        ...
        <div class="toolbar">
          <div>
            ...
            <div class="hint">自动拼接 <code class="inline">?lotCode=10037&date=YYYY-MM-DD</code></div>
          </div>
          <div>
            <label>代理前缀</label>
            <input id="proxy" placeholder="https://your-proxy.example.com/ 或 https://your-proxy.example.com/{url}">
          </div>
          <div>
            <label>半衰期（期数）</label>
        ...
        <div class="toolbar">
          <div><label><input id="autoToggle" type="checkbox"> 启用自动更新</label></div>
          <div>
            <label>频率（秒）</label>
        ...
        <div class="toolbar">
          <button id="go" class="primary">立即预测</button>
          <button id="reconcile">最新期对账</button>
          <button id="clear">清空日志</button>
        </div>

        <div id="resultCard" style="display:none;margin-top:12px">
          <div><b>下一期预测（冠军位）</b></div>
        ...
        <div id="errCard" style="display:none;margin-top:10px">
          ...
          <div class="hint">常见原因：1) CORS 2) 当天暂无数据 3) 返回结构不同 4) 代理未正确转发</div>
        </div>

        <!-- 粘贴 JSON -->
        <div id="pasteBox" style="display:none;margin-top:12px">
          <label>手动粘贴返回 JSON（原样粘贴）</label>
          <textarea id="jsonArea" rows="6" placeholder='把 https://api.api68.com/... 返回的完整 JSON 粘这里'></textarea>
          <div class="hint">在“手动粘贴 JSON”模式下，按钮会直接使用这里的 JSON。</div>
        </div>
      </section>
      ...
      <section class="card">
        <details class="hist" id="histDetails">
          <summary>
            ...
            <b>当天历史记录</b>
            <span class="hint">（默认显示最近 20 条，展开可查看全部）</span>
          </summary>
          <div class="toolbar" style="margin-top:10px">
            ...
            <div style="max-width:260px">
              <label>搜索（按期号/时间包含）</label>
              <input id="histSearch" placeholder="例如 2025-08 或 期号片段">
            </div>
          </div>
          <div class="histWrap">
            <table class="grid" id="historyTbl">
              <thead><tr><th>期号</th><th>时间</th><th>冠军</th><th>前十</th></tr></thead>
              <tbody></tbody>
            </table>
          </div>
        </details>
    ...
    <aside class="sidebar">
      <section class="card" id="logCardDesktop">
        <b>命中日志（localStorage）</b>

<div id="riskBox" class="hint" style="margin-top:6px">
  当前状态：-- ｜ 当前下注：-- ｜ 总盈亏：0 ｜ 最大回撤：0 ｜ 连挂：0
</div>

<div id="logStats" class="hint" style="margin-top:6px">
  总期数：0 ｜ 最大连中：0 ｜ 最大连挂：0
</div>
        <div class="hint" style="margin-top:4px">随时可见；手机端请点右上角“查看命中日志”。</div>
        <div style="max-height:calc(100vh - 220px);overflow:auto;margin-top:8px">
          <table class="grid" id="logTbl">
            <thead><tr><th>时间</th><th>预测期号</th><th>计划</th><th>1</th><th>命中</th></tr></thead>
            <tbody></tbody>
          </table>
        </div>
        <div class="hint">键名：<code class="inline">pks_logs_10037</code></div>
      </section>
    </aside>
  </div>
</div>
...
<div class="drawer" id="drawer">
  ...
  <div class="bd">
    <table class="grid">
      <thead><tr><th>时间</th><th>预测期号</th><th>计划</th><th>实际冠军</th><th>命中</th></tr></thead>
...
// ===== 风控与状态配置 =====
const OBSERVE_ROUNDS = 3;          // 观察局数
const OBSERVE_BET = 5;             // 观察局投注
const BET_SEQ = [5,10,20,40,80,160,320];
const MISS_WARN = 5;               // 连挂预警阈值（可改）
...
function calcRiskFromLogs(logs){
  ...
  function renderHistoryFiltered(){
    ...
    let arr = lastRecs.slice(); // 升序已在 compute 中做
    if(q){
    ...
    const fill = (tb, rows)=>{
      ...
      rows.slice().reverse().forEach(l=>{
        ...
        tr.innerHTML = `
          ...
          <td>${(l.plan||[]).join(',')}</td>
          <td>${l.actual??'-'}</td>
          <td>${l.actual==null? '待开奖' : `<span class="pill ${hit?'hit':'miss'}">${hit?'命中':'未中'}</span>`}</td>
        `;
        tb.appendChild(tr);
      });
    };
...
if(statBox){
  statBox.textContent =
    `总期数：${stats.total} ｜ 最大连中：${stats.maxHit} ｜ 最大连挂：${stats.maxMiss}`;
}
...
if(box){
  ...
  function buildFinalURL(){
    ...
    if(mode==='proxy'){
      ...
      if(!px) throw new Error('代理模式需填写“代理前缀”。\n示例：1) https://your-proxy.example.com/  2) https://your-proxy.example.com/{url}');
  ...
  async function fetchJSONWithDetails(url){
    try{
      ...
      if(!res.ok) throw new Error(`HTTP ${res.status} ${res.statusText}\nURL: ${url}\n响应体: ${text.slice(0,500)}`);
      try{ return JSON.parse(text) }catch(e){ throw new Error(`JSON 解析失败：${e.message}\nURL: ${url}\n片段: ${text.slice(0,500)}`) }
    }catch(e){
      if(String(e).includes('Failed to fetch')) throw new Error(`网络/跨域失败（可能 CORS）\n${e.message}\nURL: ${url}\n建议：代理或粘贴 JSON。`);
      throw e;
    }
  }
  ...
  function computeAndRender(recs,k,halfLife,alpha){
    if(!recs.length) throw new Error('没有解析到历史数据（检查日期/返回结构）');
    // 升序（旧→新）
    ...
    $('nextIssue').textContent=`预测期号：${String(nextIssue)}（基于最近 ${recs.length} 期）`;
    $('plan').innerHTML=picks.map(x=>`<span>${String(x.num).padStart(2,'0')}</span>`).join('');
    $('params').textContent=`K=${k}，半衰期=${halfLife}，α=${alpha.toFixed(2)}；结果= α·频次 + (1-α)·Markov(最新一期条件分布)`;
    $('latest').textContent= latest ? `最新期：${String(latest.issue)}，冠军：${latest.champ}` : '-';
  ...
  async function pullPredictReconcile(){
    ...
    try{
      ...
      if(!/^\d{4}-\d{2}-\d{2}$/.test(date)) throw new Error('日期格式应为 YYYY-MM-DD，例如 2025-08-21');
      ...
      let json=null;
      if(mode==='paste'){
        const txt=$('jsonArea')?.value?.trim?.()||''; if(!txt) throw new Error('请粘贴 JSON 再开始计算。');
        try{ json=JSON.parse(txt); }catch(e){ throw new Error('粘贴的 JSON 解析失败：'+e.message); }
      }else{
  ...
  $('reconcile').addEventListener('click', async ()=>{
    ...
    try{
      ...
      if(mode==='paste'){
        const txt=$('jsonArea')?.value?.trim?.()||''; if(!txt) throw new Error('请粘贴 JSON 再对账。');
      ...
      if(!recs.length) throw new Error('没有解析到历史数据');
      recs.sort((a,b)=> (a.time||'').localeCompare(b.time||'') || String(a.issue).localeCompare(String(b.issue)));
      const latest=recs[recs.length-1];
      ...
      saveLogs(logs); renderLogs();
      setState('ok',`对账完成：命中 ${hit}，未中 ${miss}`);
      setLastOk(Date.now());
    }catch(e){
      errCount++; setErrCnt(errCount);
      showErr(e);
